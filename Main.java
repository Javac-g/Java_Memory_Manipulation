...
public class CryptoPluginsMatrixHybrid {
    ...
    public static void register(CryptoPlugin p){
        REG.put(p.name(), p);
    }
    ...
    public static CryptoPlugin get(String k){
        return REG.get(k);
    }
    ...
    static String hex(byte[] x){
        ...
        for(byte b:x) sb.append(String.format("%02x",b&0xFF));
        return sb.toString();
    }
...
static class Exporters {
    ...
    static String motorolaAuto(byte[] data,int base){
        ...
        int max=base+Math.max(0,data.length-1);
        int type,ab;
        ...
        for(int i=0;i<data.length;i+=rec){
            int len=Math.min(rec,data.length-i), addr=base+i, bc=len+ab+1, sum=bc; sb.append('S')
                    .append(type)
                    .append(String.format("%02X",bc));
        ...
        return switch(type){
            ...
            default->sb.append("S70500000000FA\n").toString();
        };
    }
}
...
static class Meta {
    ...
    static class HybridCrypto {
        ...
        static CryptoPlugin plugin = CryptoPluginsMatrixHybrid.get("builtin");
    ...
    static byte[] readAll(InputStream in) throws IOException {
        ...
        byte[] buf=new byte[8192]; int r; while((r=in.read(buf))!=-1) bos.write(buf,0,r);
